...
class Metro(Connector):
    """epycerie.connectors.metro_connector
        ~~~~~~~~~~~~

        This module implements the connector for Metro.

        :copyright: (c) 2023 by Maxim Bacar.
        :license: Apache2, see LICENSE for more details.
    """

    __BASE_URL      = "https://www.metro.ca"
    __FLYER_URL     = "https://www.metro.ca/epicerie-en-ligne/recherche-page-{}?sortOrder=name-asc&filter=%3Arelevance%3Adeal%3ACirculaire+et+promotions"
    __ALL_URL       = "https://www.metro.ca/epicerie-en-ligne/recherche-page-{}?&filter="
    ...
    def __get_cookies( url : str ) -> dict:
        ...
        :rtype:     dict
        """
        response    = requests.get(url.format(0), headers=Metro.__HEADERS)
    ...
    def __get_page_products( page : int, headers : dict, url : str) -> list[dict]:
        ...
        for product in products:
            ...
            product_data["store"]       = Metro.__STORE
            ...
            if product.find("span", class_ = "price-update pi-price-promo"):
                product_data["sale_price"]      = Metro._format_price(product.find("span", class_="price-update pi-price-promo").text)
                product_data["regular_price"]   = Metro._format_price(product.find("div",  class_="pricing__before-price").find_all("span")[1].text)
            else:
                product_data["regular_price"]   = Metro._format_price(product.find("span", class_="price-update").text)
    ...
    def __get_products( url : str) -> list[dict]:
        ...
        headers             = Metro.__HEADERS.copy()
        headers["Cookie"]   = Metro.__get_cookies( url )

       
       
        for page in range( 1, Metro.__get_number_of_pages( headers, url ) + 1):
            
            products += Metro.__get_page_products( page, headers, url )
    ...
    def flyer():
        ...
        :rtype:     list[dict]
        """
        url = Metro.__FLYER_URL
        return Metro.__get_products( url )
